...
use crate::{config::config_loader::{get_adventurers_secret_env, get_guild_commanders_secret_env}, domain::repositories::{
    adventurers::AdventurersRepository, guild_commanders::GuildCommandersRepository,
}, infrastructure::{argon2_hashing, jwt_authentication::{self, authentication_model::LoginModel, jwt_model::{Claims, Passport, Roles}}}};
    ...
    T2: GuildCommandersRepository + Send + Sync,
{
    ...
    pub async fn adventurers_login(&self, login_model : LoginModel) -> Result<Passport> {
        ...
        let adventurer = self.adventurers_repository.find_by_username(login_model.username.clone()).await?;
