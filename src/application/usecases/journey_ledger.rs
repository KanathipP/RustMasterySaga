...
use crate::domain::{repositories::{journey_ledger::JourneyLedgerRepository, quest_viewing::QuestViewingRepository}, value_objects::{quest_adventurer_junction::MAX_ADVENTURERS_PER_QUEST, quest_statuses::QuestStatuses}};
...
T2: QuestViewingRepository + Send + Sync
{
    ...
    pub async fn in_journey(&self, quest_id: i32, guild_commander_id: i32) -> Result<i32> {
        ...
        let conditions_to_update = (quest.status == QuestStatuses::Open.to_string() || quest.status == QuestStatuses::Failed.to_string())
        && adventurers_count > 0
        ...
        let result = self.journey_ledger_repository.in_journey(quest_id, guild_commander_id).await?;
    ...
    pub async fn to_completed(&self, quest_id: i32, guild_commander_id: i32) -> Result<i32> {
        ...
        let result = self.journey_ledger_repository.to_completed(quest_id, guild_commander_id).await?;
    ...
    pub async fn to_failed(&self, quest_id: i32, guild_commander_id: i32) -> Result<i32> {
        ...
        let result = self.journey_ledger_repository.to_failed(quest_id, guild_commander_id).await?;
